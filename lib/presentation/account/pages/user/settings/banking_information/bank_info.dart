...
import '../../../../../../common/helpers/selectBankHandler.dart';
...
class editBankinginfoScreen extends StatefulWidget {
  static String id = 'inAppBanking_screen';
  const editBankinginfoScreen({super.key});

  @override
  State<editBankinginfoScreen> createState() => _inAppBankingdetailsScreenState();
}

class _inAppBankingdetailsScreenState extends State<editBankinginfoScreen> {
  ...
  Future<void> _initialSetup() async {
    String cano = await fetchBankNo();
    String cana = await fetchBankName();
    setState(() {
      currentAccNo = cano;
      currentBankName = cana;
      accountNoEditingController.text = cano;
      isLoaded = true;
    });
  }

  Future<String> fetchBankNo() async{
    try{
      FirebaseService firebaseService = FirebaseService();
      String userId = userProvider.currentUser?.userId;
      DocumentSnapshot currentUserDoc =
      await firebaseService.fetchData('UserInformation', userId);
      String accountNo;
      if (currentUserDoc.exists) {
        accountNo = currentUserDoc['Account Number'];
        print('account number: $accountNo');
        return accountNo;
      }
    }
    catch(ex){
      print('Unable to fetch bank No. $ex');
    }
    return 'Couldnt fetch bank No';
  }

  Future<String> fetchBankName() async{
    try{
      FirebaseService firebaseService = FirebaseService();
      String userId = userProvider.currentUser?.userId;
      DocumentSnapshot currentUserDoc =
      ...
      String bankName;
      if (currentUserDoc.exists) {
        bankName = currentUserDoc['Bank'];
        print('bank name: $bankName');
        return bankName;
      }
    }
    catch(ex){
      print('Unable to fetch bank Name. $ex');
    }
    return 'Couldnt fetch bank Name';
  }
                            ...
                            if(accountName.isNotEmpty && accountNumber.isNotEmpty && bankName.isNotEmpty){
                              ...
                              try {
                                ...
                                showTopSnackBar(
                                  context: context,
                                  title: 'Error:',
                                  message: 'Failed to update user Banking Information',
                                );
                                print('Failed to update Banking Information: $e');
